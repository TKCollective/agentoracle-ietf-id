...
**Status:** Internet-Draft (working manuscript, pre-submission)
**Filename:** draft-krausz-verification-state-00
**Category:** Informational
**IPR:** trust200902
**Submission type:** IETF (individual)
**Target submission:** datatracker.ietf.org/submit, June 2026
**Working group target:** RATS (cross-post)
...
8. Relationship to Related Work
   ...
   8.4. VAP Framework
   8.5. Mastercard Verifiable Intent
...
Both families produce a binary fail-closed gate primitive. They differ in predicate shape (boolean vs probabilistic) and threshold provenance (fixed vs calibration-anchored). This difference is sufficient to warrant a sibling family rather than a member entry under `environment.*`: probabilistic predicates require calibration discipline that boolean predicates do not, and the version-binding mechanism described in §4 is specific to mappings between probabilistic primitives and binary outputs.
...
- `v_calibration` (object, OPTIONAL) — Calibration anchor metadata. See §7.
...
|---|---|---|---|---|
| `supported` | `≥` | `resilient` or `not_checked` | `confident_supported` | `act` |
| `supported` | (any) | `vulnerable` | `vulnerable_supported` | `halt` |
| `supported` | `<` | `resilient` or `not_checked` | `weak_supported` | `halt` |
| `refuted` | (any) | (any) | `refuted` | `halt` |
| `unverifiable` / `unknown` | (any) | (any) | `unverifiable` | `halt` |
| (error condition) | — | — | `error` | `halt` |
...
Mastercard's Verifiable Intent specification [VINTENT] defines the `environment.*` constraint family for pre-action attestation of environment state. This document extends the constraint-family pattern to probabilistic predicates with calibration discipline, as a sibling family rather than a member entry. See §3.1.
...
**Confidence inflation attacks.** A verifier issuer who inflates `v_confidence` to push more receipts to `act` would diverge from their published calibration anchor. The reproducibility mandate (§7.1) makes this detectable. RPs SHOULD periodically sample receipts against the issuer's published harness.

**Selective disclosure.** Hash-only claim binding (§4.4) permits PII-sensitive verification without exposing claim text in the receipt envelope.

**Downgrade attacks.** A future relying party MUST NOT accept a v0.3-spec receipt against a v0.4-spec gate. The receipt format version is implicitly bound by `v_gate_mapping`; mismatches between expected and signed mapping IDs are malformed-receipt conditions per §4.3.
...
**Well-known URI:** Verification issuers using HTTPS SHOULD publish their JWKS at `/.well-known/jwks.json` per existing RFC 7517 §4.7 conventions. No new well-known URI is requested.
...
- [ANTHROPIC-ZT] Anthropic, "Zero Trust for AI Agents: A security framework for deploying autonomous AI agents in the enterprise", May 2026. https://www.anthropic.com (PDF, 36 pages).
- [AVERITEC] Schlichtkrull, M., et al., "AVeriTeC: A Dataset for Real-World Claim Verification with Evidence from the Web", 2024.
- [EU-AI-ACT] European Union, Regulation (EU) 2024/1689 of the European Parliament and of the Council on AI, Article 12 (Record-keeping). High-risk AI system obligations applicable from August 2, 2026.
...
- [VAP-FW] Kamimura, K. et al., "Verifier Abstraction Pattern Framework", draft-kamimura-vap-framework (work in progress; version pinned at submission).
- [VC-CM] World Wide Web Consortium, "Verifiable Credentials Confidence Method", W3C Working Draft.
- [VINTENT] Mastercard, "Verifiable Intent Specification", draft-msebenzi-environment-state (work in progress).
- [AO-RECEIPT-SPEC] Krausz, J., "AgentOracle Verification Receipt Format, v0.3", https://github.com/TKCollective/agentoracle-receipt-spec/tree/v0.3-binary-halt
- [AO-MAPPING-v0.3.0] Krausz, J., "AgentOracle Mapping Document v0.3.0-2026-05-30", https://github.com/TKCollective/agentoracle-receipt-spec/blob/v0.3-binary-halt/mappings/v0.3.0-2026-05-30.md
- [AO-EVAL] TKCollective, "AgentOracle Evaluation Harness (MIT-licensed, reproduces 57.6% AVeriTeC)", https://github.com/TKCollective/agentoracle-eval-harness
- [AO-VERIFY] TKCollective, "AgentOracle Receipt Verifier (open-source TypeScript)", https://github.com/TKCollective/agentoracle-receipt-verify
- [AO-BENCH] TKCollective, "AgentOracle Cross-Operator Benchmark v0.1", https://github.com/TKCollective/agentoracle-benchmark
...
- §8.4 VAP citation: pin the exact `-NN` version of draft-kamimura-vap-framework at submission time. Read the current revision before leaning on it; do not anchor on secondhand description.
- §10 IANA: coordinate with draft-msebenzi-environment-state authors before submission regarding joint namespace registration request.
- Pre-submission second-round review requested from Beenz / @headlessoracle per their May 30 commitment.
- Convert to xml2rfc v3 format for Datatracker submission. Tooling: `kramdown-rfc` or `mmark` to convert this Markdown to xml2rfc v3; validate with `xml2rfc --v3 --text --html` before upload.
- Submit via https://datatracker.ietf.org/submit ; target date June 6, 2026 (slip to mid-June acceptable if Beenz review surfaces substantive changes).
- After submission: post to RATS WG mailing list announcing the draft as an individual submission with cross-post intent.
